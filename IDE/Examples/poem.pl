...
/*  PORTABILITY:
    ...
    (1)  This code uses 'not', not '\+'.

    (2)   Operator precedences are as for PDP-11 Prolog (maximum precedence
          of 255, not 1200.
*/
...
subclass_translations :-
    ...
    assert_once( (SubCCall :- SuperCCall) ),
    assert_once( (class SubCDesc
              checks
...
assert_checks( ClassName, Checks ) :-
    assert_once(( instance(ClassName) :- Checks )).
...
assert_once((Head:- Body)):-
    clause(Head,Body),!.
assert_once(Head):-
    clause(Head,true),!.
assert_once(Term):- assertz(Term).

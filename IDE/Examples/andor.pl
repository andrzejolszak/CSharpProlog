...
printm(M,Substs,or(C,A)) :- !,
	N is M+1,
	write('or('), printm_nl(N,Substs,C), put(','),
	printm_nl(N,Substs,A), put(')').
printm(M,Substs,and(G,O)) :- !,
	N is M+1,
	write('and('), printm_nl(N,Substs,G), put(','),
	printm_nl(N,Substs,O), put(')').
printm(M,Substs,[X|L]) :- !,
	N is M+1,

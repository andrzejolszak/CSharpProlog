...
%   File   : list_utilities.pl
%   Author : Bob Welham, Lawrence Byrd, and R.A.O'Keefe
%   Updated: 7 February 2001
%   Purpose: list processing utilities

% Extended with some higher-order utilities by Neil Smith, 2001

%   This module requires
%	select/3	(from set_utilities.Pl) for perm/2
%	listtoset/2	(from set_utilities.Pl) for remove_dups/2

:- module(list_utilities, 
      [ correspond/4,			%   Elem <- List x List -> Elem
	delete/3,			%   List x Elem -> List
	removeallL/3,			%   Elem x List -> List
	last/2,				%   List -> Elem
	...
	zip/3,				%   List x List -> List
	zip_with/4			%   ListA x ListB x (a x b -> c) -> ListC
      ]).

:- ensure_loaded('.\Examples\set_utilities.pl').
:- ensure_loaded('.\Examples\higher_order.pl').

%:- index(zip/3, [1,2,3]).	% optimization imperative at end of file (LPA Prolog specific)
%:- index(zip_with/4, [1,2,4]).	% optimization imperative at end of file (LPA Prolog specific)
...
%    removeallL(Elems, List, Residue)
%    is true when Elems and List are lists, and Residue is a copy of 
%    List in which all occurrences of each element in Elems have been removed
%    Members of Elems need not occur in List
...
sumlist(Numbers, Total):-
	foldl1(Numbers, [X, Y, Z]^(Z is X + Y), Total).


%   zip(Xs, Ys, XYs)
%   is true if each XY = X - Y
